...
struct ClientArgs {
    #[arg(
        long,
        default_value = "wss",
        help = "The protocol to use when connecting to the host"
    )]
    protocol: String,
    #[arg(
        short,
        long,
        default_value = "clickertrain.ing",
    ...
    #[arg(
        short,
        long,
        default_value = "~/.config/clickrtraining/sounds/",
        help = "A directory holding custom sound files, all ending in .ogg"
    )]
...
async fn main() -> Result<()> {
    ...
    match Cli::parse().command {
        ...
        Command::Click(args) => {
            ...
            match args.sound {
                Some(sound) => url.add_route("custom").add_route(&sound.replace("/", "_")),
